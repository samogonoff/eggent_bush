...
export function createAgentTools(
  ...
  if (settings.codeExecution.enabled) {
    ...
    tools.mssql = tool({
      description:
        "Execute MS SQL queries and return results as a pandas DataFrame. Use this to query MS SQL Server databases, analyze data, or perform CRUD operations. Results are returned as a DataFrame for easy analysis.",
      inputSchema: z.object({
        connection_string: z
          .string()
          .describe("MS SQL connection string (e.g., 'Driver={ODBC Driver 17 for SQL Server};Server=localhost;Database=mydb;UID=user;PWD=password')."),
        query: z
          .string()
          .describe("SQL query to execute (SELECT, INSERT, UPDATE, DELETE)."),
        params: z
        ...
        save_to: z
          .string()
          .optional()
          .describe("Optional file path to save the result DataFrame as CSV."),
        preview_only: z
          .boolean()
          .default(false)
          .describe("If true, only analyze the query without executing. Returns query preview and warnings about dangerous operations."),
        confirm: z
          .boolean()
          .default(false)
          .describe("Must be set to true to execute INSERT/UPDATE/DELETE queries. Otherwise they will be blocked."),
      }),
      execute: async ({ connection_string, query, params, save_to, preview_only, confirm }) => {
        // Analyze query for dangerous operations
        const dangerousOps = detectDangerousSqlOperations(query);
        
        if (dangerousOps.length > 0) {
          if (preview_only) {
            return formatQueryPreview(query, dangerousOps, true);
          }
          if (!confirm) {
            return `⚠️ BLOCKED: This query contains potentially dangerous operations: ${dangerousOps.join(", ")}\n\n` +
              `Query preview:\n${query}\n\n` +
              `To execute this query, call mssql again with confirm: true\n` +
              `Example: mssql with { confirm: true, ... }`;
          }
        }
        
        if (preview_only) {
          return formatQueryPreview(query, dangerousOps, false);
        }
        
        const pythonCode = buildMssqlQuery(connection_string, query, params, save_to);
        const cwd = resolveContextCwd(context);
        return executeCode("python", pythonCode, 0, settings.codeExecution, cwd, {});
      },
    });
...
/**
 * Detect dangerous SQL operations in query
 */
function detectDangerousSqlOperations(query: string): string[] {
  ...
  const patterns = [
    ...
    { pattern: /\bDROP\s+(TABLE|DATABASE|INDEX|VIEW|PROCEDURE)\b/, name: "DROP" },
    { pattern: /\bTRUNCATE\s+TABLE\b/, name: "TRUNCATE" },
    { pattern: /\bALTER\s+(TABLE|DATABASE)\b/, name: "ALTER" },
    { pattern: /\bCREATE\s+(TABLE|DATABASE|INDEX|VIEW|PROCEDURE)\b/, name: "CREATE" },
    { pattern: /\bEXEC\b|\bEXECUTE\b/, name: "EXECUTE" },
...
/**
 * Format query preview with analysis
 */
function formatQueryPreview(
  ...
  let result = `📊 **Query Preview**\n\n`;
  result += `\`\`\`sql\n${preview}\n\`\`\`\n\n`;
  
  if (dangerousOps.length > 0) {
    result += `⚠️ **Dangerous Operations Detected:** ${dangerousOps.map(op => `\`${op}\``).join(", ")}\n\n`;
    result += `This query will **${isPreviewMode ? "ONLY ANALYZE" : "BE BLOCKED"}** without confirmation.\n`;
    result += `To ${isPreviewMode ? "execute" : "proceed"}, add \`confirm: true\` to the tool call.\n`;
  } else {
    result += `✅ **Safe:** This is a SELECT query - no data modification.\n`;
    result += `Query will be executed normally.\n`;
  }
  
  result += `\n**Full query:** ${query.length} characters, ${lines.length} lines`;
  
  return result;
}

/**
 * Build Python code to execute MS SQL query using pymssql
 */
function buildMssqlQuery(
  ...
  // Format: Driver={ODBC Driver 17 for SQL Server};Server=olap;Database=DWH;UID=sa;PWD=password
  const serverMatch = connectionString.match(/Server=([^;]+)/i);
  const databaseMatch = connectionString.match(/Database=([^;]+)/i);
  const userMatch = connectionString.match(/UID=([^;]+)/i);
  const passwordMatch = connectionString.match(/PWD=([^;]+)/i);
  ...
  if (params && params.length > 0) {
    ...
    paramsCode = `, params=(${paramValues})`;
  }

  let saveCode = "";
  if (saveTo) {
    const escapedPath = saveTo.replace(/'/g, "\\'");
    saveCode = `
# Save results to CSV
df.to_csv('${escapedPath}', index=False)
print("Results saved to ${escapedPath}")`;
  }
...
try:
    ...
    import subprocess
    subprocess.run(['python3', '-m', 'pip', 'install', 'pymssql', '-q'], check=True)
    import pymssql
...
try:
    ...
    # Execute query and load results into DataFrame using pandas
    df = pd.read_sql('${escapedQuery}'${paramsCode}, conn)
