...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        switch (provider) {
            ...
            case "opencode": {
                let baseUrl = searchParams.get("baseUrl") || "https://opencode.ai/zen/v1";
                
                // Get API key from env if not provided in params
                if (!apiKey && process.env.OPENCODE_API_KEY) {
                ...
                if (!res.ok) throw new Error(`OpenCode Zen API error: ${res.status}`);
                const data = await res.json();
                // OpenCode Zen returns models in data.data array
                const rawModels = Array.isArray(data) ? data : (data.data || []);
                models = rawModels
                    .map((m: { id: string; name?: string }) => ({
                        id: m.id,
