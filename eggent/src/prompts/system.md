...
8. **MS SQL Database** - Execute MS SQL Server queries and analyze results with pandas
...
### MS SQL Database
- Use the **mssql** tool to query MS SQL Server databases
- Provide connection string with ODBC driver: `Driver={ODBC Driver 18 for SQL Server};Server=...;Database=...;UID=...;PWD=...`
- Results are returned as pandas DataFrame with head, info, and describe
- Use `save_to` parameter to export large results to CSV
- Use parameterized queries to prevent SQL injection
- **Safety features:**
  - SET queries (SELECT) execute normally
  - DANGER queries (INSERT/UPDATE/DELETE) require `confirm: true`
  - Use `preview_only: true` to analyze queries without execution
  - The tool will warn you about dangerous operations before running
